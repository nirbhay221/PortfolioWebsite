...
export default function AnotherRGTCarAd() {
  const { nodes, materials } = useGLTF("./model/phone.glb");
  const [video] = useState(() => Object.assign(document.createElement('video'), { src: './Videos/Alpha.mp4', crossOrigin: 'Anonymous', loop: true, muted: true }))
        useEffect(() => void video.play(), [video])
  return (
   
      <group          position={[54,1.5,-15]}         rotation={[-Math.PI*1 , 0.5*Math.PI, 0.5*Math.PI]}>
