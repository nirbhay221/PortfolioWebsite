...
export default function Scifi({thirdPerson}) {
  let ref = useRef();
  const { scene, nodes, materials } = useGLTF('./model/scificar.glb')

  useLayoutEffect(() => {
  
    scene.traverse((obj) => obj.type === 'Mesh' && (obj.receiveShadow = obj.castShadow = true))
    Object.assign(nodes.UpperBody.material,{roughness : 0.1,
      metalness : 0.9,
...
useGLTF.preload('./model/scificar.glb');
