...
export default function AudiAd() {
  const { nodes, materials } = useGLTF("./model/phone.glb");
  const [video] = useState(() => Object.assign(document.createElement('video'), { src: './Videos/CarAd.mp4', crossOrigin: 'Anonymous', loop: true, muted: true }))
        useEffect(() => void video.play(), [video])
  return (
   
      <group        position={[40,1.5,-15]}         rotation={[-Math.PI*1 , 0.5*Math.PI, 0.5*Math.PI]}>
